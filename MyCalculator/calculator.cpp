...
Calculator::Calculator()
{
    ...
    m_display_down = new QLineEdit("0");
    m_sign = new QLabel("+");
    ...
    Button *divisionButton = createButton(m_division_sign, SLOT(additiveOperatorClicked()));
    Button *timesButton = createButton(m_times_sign, SLOT(additiveOperatorClicked()));
    Button *minusButton = createButton(m_minus_sing, SLOT(additiveOperatorClicked()));
    Button *plusButton = createButton(m_plus_sign, SLOT(additiveOperatorClicked()));

    Button *squareRootButton = createButton(m_squareRoot_sign, SLOT(unaryOperatorClicked()));
    Button *powerRootButton = createButton(m_power_sign, SLOT(unaryOperatorClicked()));
    ...
    mainLayout->addWidget(powerRootButton, 5,5);
...
{
    m_display_up->setText("ERR");
    m_display_down->setText("ERR");
}
...
bool Calculator::calculate(double operand, const QString &operation)
{
    ...
    {
        temp_total += operand;
    }else if (operation == m_minus_sing)
    {
        temp_total -= operand;
